...
df <- train %>%
  transmute(row_id = paste(cfips, date, sep = "_"), cfips, county, state, date, activity, active) %>%
  bind_rows(test) %>%
  arrange(cfips, date) %>%
  mutate(
...
df <- mutate(df, istest = if_else(is.na(activity), 1, 0))
...
blacklist <- c('North Dakota', 'Iowa', 'Kansas', 'Nebraska', 'South Dakota','New Mexico',
               'Alaska', 'Vermont')
blacklistcfips <- c(
...
active_threshold = 1.8   #df$lastactive
absolute_threshold = 1.00  #df$lasttarget
...
for (t in 31:38) {
  ...
  valid_indices <- df %>%
    filter((istest == 0) & (time == t))
  
  df$target[train_indices] <- aclip(df$target[train_indices], -0.0043, 0.0045)
  ...
  df$k[valid_indices] <-
    df$k[valid_indices] * df$activity[valid_indices]
  
  df <- df %>% group_by(cfips) %>%
    mutate(lastval = activity[t]) %>%
    ungroup()
  ...
  dz <- df %>% group_by(cfips) %>%
    select(cfips, time, activity, state, lastactive, lastval) %>%
    filter(time == (t + 1)) %>%
    ungroup()
  
  dz$pred <- dt$k
  
  dz$pred[dz$lastactive <= active_threshold] <- dz$lastval[dz$lastactive <= active_threshold]
  dz$pred[dz$lastval <= absolute_threshold] <- dz$lastval[dz$lastval <= absolute_threshold]
  dz$pred[dz$lastval <= absolute_threshold] <- dz$lastval[dz$lastval <= absolute_threshold]
  dz$pred[dz$state %in% blacklist]  <- dz$lastval[dz$state %in% blacklist]
  dz$pred[dz$cfips %in% blacklistcfips]  <- dz$lastval[dz$cfips %in% blacklistcfips]
  
  print(t)
  cat('xgb ', smape(dz$activity, dz$pred), '\n')
  cat('last', smape(dz$activity, dz$lastval), '\n')
  
  i <- i + 1
  all_scores[[i]] <- model$best_score
  all_rounds[[i]] <- model$best_iteration
  sxgb[[i]] <- smape(dz$activity, dz$pred)
  slast[[i]] <- smape(dz$activity, dz$lastval)
}
print(all_rounds)
print(all_scores)
print(sxgb)
...
summary(all_rounds)
imp_matrix <-
  xgb.importance(feature_names = colnames(train$data), model = model)
print(imp_matrix)

train_indices <-
  df %>% filter(istest == 0,
                time < time_series_length,
                time >= 2,
                lastactive > active_threshold,
                lasttarget > absolute_threshold) %>% select(time)

valid_indices <- df %>% filter(time == time_series_length) %>% select(row_number())
...
df$k[valid_indices] <- ypred + 1
df$k[valid_indices] <- df$k[valid_indices] * df$microbusiness_density[valid_indices]

df <- df %>% group_by(cfips) %>%
  mutate(lastval = microbusiness_density[TS]) %>%
  ungroup()
...
df <- df %>% 
  filter(cfips %in% c(28055, 48269)) %>% 
  mutate(microbusiness_density = if_else(cfips == 28055, 0, 1.762115))


test <- df %>% group_by(cfips) %>%
  filter(istest == 1) %>%
  select(row_id, cfips, time) %>%
  ungroup()

fcts <- left_join(test, df, by = c("cfips", "time")) %>%
  mutate(microbusines_dnensity = pred)

test <- select(fcts, row_id, microbusiness_density)
